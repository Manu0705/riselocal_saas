...
export default function LeadCard({ lead }: Readonly<{ lead: LeadLike }>) {
  ...
  const cycleReminderDay = (direction: 1 | -1) => {
    const currentIndex = REMINDER_DAYS.indexOf(selectedReminderDay as (typeof REMINDER_DAYS)[number])
    const safeIndex = Math.max(0, currentIndex)
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12, gap: 10 }}>
        ...
        <span style={{ fontSize: 12, color: "var(--muted)", whiteSpace: "nowrap", flexShrink: 0 }}>{createdAtLabel}</span>
      </div>
