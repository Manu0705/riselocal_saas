...
export default function ReviewsPage() {
  ...
  async function fetchReviews() {
    ...
    try {
      setLoading(true)
      const response = await api.get(`/tenant/${slugOrId}/feedback`)
      const data = Array.isArray(response) ? response : response?.data || []
      
      // Fetch lead names for each review
      const leadsResponse = await api.get(`/tenant/${slugOrId}/leads`)
      const leads = Array.isArray(leadsResponse) ? leadsResponse : leadsResponse?.data || []
      
      const reviewsWithLeads = data.map((review: any) => ({
        ...review,
        lead: leads.find((l: any) => l.id === review.leadId)
      }))
  ...
  async function handleApprove(reviewId: string) {
    try {
      await (api as any).patch(`/feedback/${reviewId}/approve`)
      // Refresh reviews
      fetchReviews()
    } catch (err) {
  ...
  async function handleReject(reviewId: string) {
    try {
      await (api as any).patch(`/feedback/${reviewId}/reject`)
      // Refresh reviews
      fetchReviews()
    } catch (err) {
