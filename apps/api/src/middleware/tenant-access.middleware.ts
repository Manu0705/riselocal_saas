...
export function tenantAccessMiddleware(req: Request, res: Response, next: NextFunction) {
  ...
  const routeTenantId = typeof request.params.tenantId === "string" ? request.params.tenantId : undefined
  ...
  if (!isPrivilegedRole(request.user?.role) && userTenantId && effectiveTenantId && userTenantId !== effectiveTenantId) {
    return res.status(403).json({
      success: false,
